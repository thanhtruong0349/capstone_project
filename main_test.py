...
#python main_test.py --weights /checkpoints/custom-416 --video input/test_video2.mp4 --interactive
...
def main(_argv):
    ...
    print("[INFO OUTPUT]: " + str(field_dict))
    ...
    # Insert ID information to MySQL database
    ...
    values = ", ".join(["%s"] * len(field_dict))
    query = f"INSERT INTO {table_name} ({columns}) VALUES ({values})"

    # execute the query with the dictionary values as parameters
    cursor.execute(query, tuple(field_dict.values()))
